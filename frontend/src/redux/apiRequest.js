...
export const loginUser = async(user, dispatch, navigate) => {
    dispatch(loginStart());
    try{
        const res = await axios.post("http://localhost:3002/v1/auth/login", user);
...
export const registerUser = async(user, dispatch, navigate) =>{
    ...
    try{
        await axios.post("http://localhost:3002/v1/auth/register", user);
...
export const logOut = async(dispatch, id, navigate, accessToken, axiosJWT) => {
    ...
    try{
        await axiosJWT.post("http://localhost:3002/v1/auth/logout",id,{
